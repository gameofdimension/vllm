...
5. ✅ **decode-step profile（诊断完成 2026-06-22）**：torch profiler 解析 rank0 decode trace → **瓶颈是 `_tiled_sparse_decode_kernel`（MLA decode Triton 核，移植自 SGLang）占 88.8%** GPU 时间；Op1 1.1%、通信 4.5%、MoE 0.3%。推翻了"MoE/通信是瓶颈"的旧假设；也解释了 cudagraph 无增益（该核 eager-break）。**优化尝试**：扩 autotune（3→8 配置）独立 bench +12% 但系统 decode 无增益（已 revert）；诊断出该核 gather-memory-bound（~350× off 峰值）+ TP8 小 H 下 SM 利用不足（~43%）。**正解 = split-K（topk 切分换并行度 + LSE merge）+ gather 优化，是独立核工程**（`sm120/bench/mla_decode_bench.py` 留作迭代工具）。
