...
5. ✅ **decode-step profile（诊断完成 2026-06-22）**：torch profiler 解析 rank0 decode trace → **瓶颈是 `_tiled_sparse_decode_kernel`（MLA decode Triton 核，移植自 SGLang）占 88.8%** GPU 时间；Op1 1.1%、通信 4.5%、MoE 0.3%。推翻了"MoE/通信是瓶颈"的旧假设；也解释了 cudagraph 无增益（该核 eager-break）。**下一步：优化 MLA decode 核（扩 autotune / 调 grid / re-port SGLang 更新版）以追平 SGLang decode 2.3×。**
